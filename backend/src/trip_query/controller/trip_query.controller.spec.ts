...
it("should query trip", () => {
  ...
  vi.spyOn(app.get(TripQueryService), "queryTrip").mockResolvedValue({ alternatives: mockAlternatives });
  const requestBody = {
  ...
  const response = controller.queryTrip(requestBody);
